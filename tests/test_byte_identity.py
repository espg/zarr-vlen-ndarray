...
def test_chunk_bytes_identical_to_vlen_bytes(
    ...
    if case == "float32_2col":
        ...
        cells = uint64_cells
        zdtype = NDArray(dtype="uint64", shape=(None,))
    n = len(cells)
...
def test_relabel_bytes_store_reads_identically(tmp_path, float32_cells):
    """A vlen-bytes store's chunk objects, re-labeled ndarray + vlen-ndarray
    via a metadata-only zarr.json rewrite, read back as the typed cells."""
    import json
    ...
    assert isinstance(reread.metadata.data_type, NDArray)
